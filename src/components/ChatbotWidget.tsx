...
const botReply = (input: string) => {
  ...
  if (/(hi|hello|salam|assalam|hey)/.test(q)) {
    return "Hello! I am the Sun Dental - Dr. Ashish Ranjan assistant. You can ask about implants, pain, braces, cleaning, timings, or booking.";
  }
  ...
  if (/(pain|dard|toothache|sujaan|swelling)/.test(q)) {
    return "If you have severe pain or swelling, do not delay. Avoid very hot or cold foods and visit the clinic for an emergency checkup, or call: 098118 28767.";
  }
  ...
  if (/(appointment|book|booking|schedule)/.test(q)) {
    return "To book an appointment, fill the Book Now form on this page or call directly: 098118 28767.";
  }

  if (/(address|location|map|kahan)/.test(q)) {
    return "Clinic location: Delhi, India. Call us for nearby landmark details.";
  }
  ...
  if (/(emergency|urgent|bleeding|broken tooth)/.test(q)) {
    return "In an emergency, contact us immediately: 098118 28767. We handle urgent cases including dental pain, fractures, and infections.";
  }
